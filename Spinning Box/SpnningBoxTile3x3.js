...
function setUpTextures()
{
    ...
    // Get image
    image512 = document.getElementById("pic1");
    image500 = document.getElementById("pic2");
    image256 = document.getElementById("pic3");
    ...
    gl.texImage2D(gl.TEXTURE_2D,     // 2D texture
                 ...
                 image512              // image itself - size is determined automatically
                 );
...
document.onkeydown = function handleKeyDown(event) {
   ...
   var key = String.fromCharCode(c);
    ...
    {
    	pictureNumber++;
    	console.log("pictureNumber: "+pictureNumber%3);
    	switch(pictureNumber%3){
    		case 0:    			
				gl.texImage2D(gl.TEXTURE_2D,     // 2D texture
							 0,                 // level of detail 0 (full or no mipmap)
							 gl.RGB,	        // internal format - how GL will store tex.
				...
				gl.bindTexture(gl.TEXTURE_2D, textures[0]);
    			document.getElementById("picSize").innerHTML = "image size is 512x512";
    			break;
    		case 1:    			
    			gl.texImage2D(gl.TEXTURE_2D,     // 2D texture
							 0,                 // level of detail 0 (full or no mipmap)
							 ...
							 gl.RGB,            // format of image data in memory
							 gl.UNSIGNED_BYTE,  // data type of image data in memory
							 image500             // image itself - size is determined automatically
				);
				gl.bindTexture(gl.TEXTURE_2D, textures[1]);
    			document.getElementById("picSize").innerHTML = "image size is 500x500";
    			break;
			case 2:			
				gl.texImage2D(gl.TEXTURE_2D,     // 2D texture
							 0,                 // level of detail 0 (full or no mipmap)
							 ...
							 gl.RGB,            // format of image data in memory
							 gl.UNSIGNED_BYTE,  // data type of image data in memory
							 image256             // image itself - size is determined automatically
				);
				gl.bindTexture(gl.TEXTURE_2D, textures[2]);
				document.getElementById("picSize").innerHTML = "image size is 256x256";
				break;
    	}
    }
}